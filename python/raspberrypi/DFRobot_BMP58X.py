...
class DFRobot_BMP58X(object):
    ...
    OOR_COUNT_LIMIT_1 = 0x00
    OOR_COUNT_LIMIT_3 = 0x01
    OOR_COUNT_LIMIT_7 = 0x02
    OOR_COUNT_LIMIT_15 = 0x03

    SLEEP_MODE = 0x00  # // 待机模式
    NORMAL_MODE = 0x01  # // 普通模式
    SINGLE_SHOT_MODE = 0x02  # // 强制模式 > 只进行一次
    CONTINOUS_MODE = 0x03  # // 连续模式
    DEEP_SLEEP_MODE = 0x04  # // 深度待机模式

    # /*! Fifo disabled */
    FIFO_DISABLED = 0x00
    # v/*! Fifo temperature data only enabled */
    FIFO_TEMPERATURE_DATA = 0x01
    # /*! Fifo pressure data only enabled */
    FIFO_PRESSURE_DATA = 0x02
    # /*! Fifo pressure and temperature data enabled */
    FIFO_PRESS_TEMP_DATA = 0x03

    FIFO_NO_DOWNSAMPLING = 0x00
    FIFO_DOWNSAMPLING_2X = 0x01
    FIFO_DOWNSAMPLING_4X = 0x02
    FIFO_DOWNSAMPLING_8X = 0x03
    FIFO_DOWNSAMPLING_16X = 0x04
    FIFO_DOWNSAMPLING_32X = 0x05
    FIFO_DOWNSAMPLING_64X = 0x06
    FIFO_DOWNSAMPLING_128X = 0x07

    FIFO_STREAM_TO_FIFO_MODE = 0x00
    FIFO_STOP_ON_FULL_MODE = 0x01

    INT_STATUS_DRDY = 0x01                   # // 数据就绪
    INT_STATUS_FIFO_FULL = 0x02              # // FIFO已满
    INT_STATUS_FIFO_THRES = 0x04             # // FIFO达到阈值
    INT_STATUS_PRESSURE_OOR = 0x08           # // 气压超出测量范围
    INT_STATUS_POR_SOFTRESET_COMPLETE = 0x10  # // 上电复位/软复位完成

    INT_DATA_DRDY = 0x01
    INT_FIFO_FULL = 0x02
    INT_FIFO_THRES = 0x04
    INT_PRESSURE_OOR = 0x08

    INT_MODE_PULSED = 0x00
    INT_MODE_LATCHED = 0x01

    INT_POL_ACTIVE_LOW = 0x00
    INT_POL_ACTIVE_HIGH = 0x01

    INT_OD_PUSH_PULL = 0x00
    INT_OD_OPEN_DRAIN = 0x01
    ...
    def _verify_deep_standby_mode(self):
        ...
        if odr > self.ODR_05_HZ and fifo_frame_sel == self.FIFO_DISABLE and iir_t == self.IIR_FILTER_BYPASS and iir_p == self.IIR_FILTER_BYPASS:
            mode = self.SLEEP_MODE
        return mode
...
class DFRobot_BMP58X_UART(DFRobot_BMP58X, DFRobot_RTU):
    ...
    def _write_holding_reg(self, reg, data):
        ...
        ret = self.write_holding_registers(self.__addr, reg + self.OFFREGSET, tmp_data)
        return ret

    def _read_input_reg(self, reg, size):
        try:
            data = self.read_input_registers(self.__addr, reg + self.OFFREGSET, size)
            ret_data = []
            for i in range(size):
    ...
    def _read_holding_reg(self, reg, size):
        try:
            data = self.read_holding_registers(self.__addr, reg + self.OFFREGSET, size)
            ret_data = []
            for i in range(size):
                ret_data.append((data[i*2+1] << 8) | data[i*2+2])
            return ret_data
